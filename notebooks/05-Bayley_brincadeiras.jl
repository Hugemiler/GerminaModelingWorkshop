...
numero_de_epocas = 500 # numero de vezes que o modelo vai ver o dado
...
modelo = Flux.Chain(
    Dense(length(input_cols) => 8, sigmoid),
    Dense(8 => 4, sigmoid),
    Dense(4 => 1, identity),
...
decay_opt_state = Flux.setup(Flux.OptimiserChain(Flux.Optimise.WeightDecay(0.001), Flux.Optimise.Adam(0.001)), modelo)
...
for epoch in 1:numero_de_epocas
  Flux.train!(modelo, microbiome_dataloader, decay_opt_state) do m, x, y
    loss(m(x), y)
  end
end
